...
function CheckWorkTime() {
  ...
  .get().then((snapshot)=> {
    snapshot.forEach(doc=>{
      ...
      $("#UserTimeIN").html('<div class="time-header">เวลาเข้างาน<br>ก่อนเวลา '+ doc.data().WorkTimeIN + ' น.</div>');
      $("#UserTimeOUT").html('<div class="time-header">เวลาออกงาน<br>หลังเวลา '+ doc.data().WorkTimeOUT + ' น.</div>');
...
function ShowTimeing() {
  ...
  var datetoday = "วัน"+ dayNames[m.getDay()] + "ที่ " +
      ("0" + m.getDate()).slice(-2) + " " + monthNames[m.getMonth()+1] + " " +
